...
router.get("/user/:user_id", (req, res) => {
  ...
  Profile.find({ createdBy: req.params.user_id })
    ...
    .catch(err =>
      res
        .status(404)
        .json({ profile: "there are no profiles created by this user" })
    );
});
...
router.get("/search/:name", (req, res) => {
  ...
  Profile.find({ name: req.params.name })
    ...
    .catch(err =>
      res.status(404).json({ profile: "there are no profiles with this name" })
    );
});
...
router.post(
  ...
  (req, res) => {
    ...
    Profile.findOne({ createdBy: req.user.id, _id: req.body.id }).then(
      profile => {
        if (profile) {
          // UPDATE PROFILE
